...
def get_field(tokenizer: Tokenizer, fields: tuple = None, use_test: bool = False) -> (Field, Field, RawField):
    ...
    if fields:
        ...
        for feat, vocab in tokenizer.feat_vocabs.items():
            feat_field = Field(tokenize=lambda x: x, init_token=None, eos_token=None,
                               pad_token='<PAD>', unk_token='<PAD>', lower=False, batch_first=True,
...
class NMTDataset(object):
    def __init__(self, src_file: str, tgt_file: str, fields: tuple, cache: tuple, tokenizer: Tokenizer,
                 features: list = ['deprel'], min_len: int = 1, max_len: int = 256,
        ...
        self.dataset = self.__read_data(src_file, tgt_file, cache)

    def __read_data(self, src_file: str, tgt_file: str, cache: tuple = None) -> (list, list):
        if cache:
            return Dataset(cache[0], cache[1])
        ...
        for src, tgt in tqdm(list(zip(srcs, tgts))):
            ...
            if self.min_len <= src_len <= self.max_len and \
                ...
                sents_feats = self.tokenizer.get_feature(src)
                for feat_name, feat in sents_feats.items():
                    data.append(feat)
